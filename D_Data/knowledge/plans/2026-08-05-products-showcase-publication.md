...
- [x] Write a failing source test asserting exactly `signal-newsroom` and `digital-goods-store`, stable `/products/...` routes, and rejection of traversal/private paths.
- [x] Run `python -B -m unittest P_Process.tests.test_products_source -v`; expect missing loader failure.
- [x] Add the schema, manifest, and minimal loader with explicit allow-list validation.
- [x] Rerun the test; expect PASS.
- [x] Commit `feat(products): add canonical showcase contract`.
...
- [x] Write a failing package test for semantic pages, one `h1`, implemented/target architecture, local assets, and forbidden path/secret patterns.
- [x] Run the test; expect missing packages failure.
- [x] Export only the verified public artifacts from the Fullstack Agent product outputs and normalize links for `/products/` mounting.
- [x] Rerun the test; expect PASS.
- [x] Commit `feat(products): add two verified showcase packages`.
...
- [x] Write failing deterministic build/composition tests for gallery, both packages, `/writing/` preservation, and collision rejection.
- [x] Run the tests; expect missing builder failure.
- [x] Implement allow-listed build, gallery shell, and atomic compositor mount.
- [x] Rerun owning and composition tests; expect PASS.
- [x] Commit `feat(products): build isolated showcase destination`.
...
- [x] Write failing registry and validation tests for active ownership, route uniqueness, package completeness, and no private strings.
- [x] Run owning tests; expect planned/placeholder and missing validator failures.
- [x] Activate registry and add the read-only audit gate to `run_all`.
- [x] Rerun owning tests and `python -B -m P_Process.validation.run_all`; expect PASS.
- [x] Commit `feat(products): activate products publication gates`.
...
- [x] Run full Python, Node, validation, syntax, and browser-contract suites.
- [x] Build with `python -B -m P_Process.build.build_site --destination C:\tmp\dnh-products-site` and inspect routes/resources; manual browser inspection remains pending because host browser tooling is unavailable.
- [x] Record only observed evidence; run `git diff --check` and verify clean scoped status.
- [x] Commit `test(products): accept showcase publication`.
- [x] Merge feature branch into local `main`, rerun full gates, push `main`, then monitor `Validate` and `Deploy Pages` to success or report the exact blocker.
