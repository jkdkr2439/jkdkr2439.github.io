...
test('accepts the canonical six-module platform registry', async () => {
  const registry = JSON.parse(await readFile('D_Data/platform/registry/modules.json', 'utf8'));
  const result = validateRegistry(registry);
  assert.deepEqual(result.accepted.map(({id}) => id), ['writing', 'products', 'papers', 'books', 'media', 'connect']);
  assert.deepEqual(result.accepted.map(({id,entry,slot}) => ({id,entry,slot})), [
    ...
    {id:'books',entry:'books',slot:'books'},
