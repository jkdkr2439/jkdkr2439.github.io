...
export function createShell(root,identity,locale) {
  ...
  const media=node('section','media-panel'); media.id='media-panel'; media.hidden=true; media.setAttribute('aria-live','polite'); main.append(destinations,media);
  ...
  return {slot:name=>{if(name==='destinations')return destinations;if(name==='media')return media;throw new Error('UNKNOWN_SLOT')},showFailure:(code,id)=>footer.append(node('p','module-failure',`${id}: ${code}`)),setStatus:value=>{count=value;renderStatus(locale.get())},navigate:route=>window.location.assign(route)};
}
