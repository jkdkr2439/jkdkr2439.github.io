...
export function createShell(root,identity,locale) {
  ...
  const eyebrow=node('p','eyebrow'), name=node('h1','name',identity.name), credit=node('p','identity-credit',identity.credit || 'Kevin T.N'), statement=node('p','statement');
  const language=node('div','language-switch'); language.setAttribute('role','group'); language.setAttribute('aria-label','Language');
  const vi=node('button','language-option','VI'), en=node('button','language-option','EN'); vi.type=en.type='button'; vi.onclick=()=>locale.set('vi'); en.onclick=()=>locale.set('en'); language.append(vi,en); header.append(eyebrow,name,credit,statement,language);
  ...
  const books=node('aside','books-rail'); books.id='books'; books.hidden=true; books.setAttribute('aria-live','polite');
  const footer=node('footer',''); page.append(header,main,books,footer); root.append(page);
  ...
  return {slot:name=>{if(name==='destinations')return destinations;if(name==='connect')return connect;if(name==='media')return media;if(name==='books')return books;throw new Error('UNKNOWN_SLOT')},showFailure:(code,id)=>footer.append(node('p','module-failure',`${id}: ${code}`)),setStatus:()=>{},navigate:route=>window.location.assign(route)};
}
