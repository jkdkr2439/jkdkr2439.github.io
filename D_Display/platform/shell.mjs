...
export function createShell(root,identity,locale) {
  ...
  const main=node('main',''); const destinations=node('nav','destinations'); destinations.setAttribute('aria-label','Destinations'); main.append(destinations);
  ...
  return {slot:name=>{if(name!=='destinations')throw new Error('UNKNOWN_SLOT');return destinations},showFailure:(code,id)=>footer.append(node('p','module-failure',`${id}: ${code}`)),setStatus:value=>{count=value;renderStatus(locale.get())},navigate:route=>window.location.assign(route)};
}
